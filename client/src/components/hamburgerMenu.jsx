    ...
    <div>
      ...
      {open && (
        <motion.div
          ...
          className="fixed inset-0 w-screen h-screen bg-white shadow-lg z-50 flex flex-col"
        >
          {/* Main Menu */}
          <div className="flex flex-col items-start justify-center w-full h-full p-8 space-y-6">
            {menuItems.map((item, index) => (
              <div key={index} className="w-full">
                {item.link ? (
                  <Link
                    to={item.link}
                    className="block text-2xl font-semibold p-2 hover:text-blue-500 transition-all"
                  ...
                  <div
                    className="cursor-pointer text-2xl font-semibold p-2 hover:text-blue-500 transition-all"
                    onClick={() =>
